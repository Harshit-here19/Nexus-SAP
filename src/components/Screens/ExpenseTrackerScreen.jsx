...
const ExpenseTrackerScreen = ({ mode = 'create' }) => {
  ...
  saveRef.current = () => {
    ...
    try {
      if (mode === 'create') {
        const expNumber = generateNextNumber('expenses', 'expenseNumber', 'EXP');
  ...
  const getCategoryInfo = (categoryValue) => {
    return categories.find(c => c.value === categoryValue) || { label: categoryValue, color: '#9e9e9e' };
  };
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
        ...
        <div>
          ...
          <div className="sap-form-group">
            ...
            {errors.amount && (
              <div style={{ color: 'var(--sap-negative)', fontSize: '11px', marginTop: '4px', marginLeft: '162px' }}>
                {errors.amount}
              </div>
            )}
          </div>
      ...
      {formData.category && (
        <div style={{
          ...
          <div>
            ...
            {formData.amount && (
              <div style={{ fontSize: '18px', fontWeight: '700', color: getCategoryInfo(formData.category).color }}>
  ...
  const getModeTitle = () => {
    switch (mode) {
      case 'create': return 'Record Expense';
      case 'change': return 'Edit Expense';
      case 'display': return 'View Expense';
          ...
          {needsLoad ? (
            <div>
              ...
              <div className="sap-form-row" style={{ display: 'flex', alignItems: 'flex-end', gap: '10px' }}>
                ...
                <SapButton onClick={() => {
                  setSearchResults(getTableData('expenses').sort((a, b) => new Date(b.date) - new Date(a.date)));
                  setShowSearchModal(true);
                }} icon="🔎">
                  Search
                </SapButton>
              ...
              {searchResults.length === 0 ? (
                ...
                searchResults.map((expense, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <SapButton onClick={() => handleSelectExpense(expense)} type="primary">
                        Select
                      </SapButton>
                    </td>
                  </tr>
        ...
        {searchResults.length > 0 && (
          <div style={{
            ...
            <span style={{ fontWeight: '600' }}>
              Total: USD {searchResults.reduce((sum, e) => sum + parseFloat(e.amount || 0), 0).toFixed(2)}
            </span>
          </div>
        )}
      </SapModal>
