...
const MaterialMasterScreen = ({ mode = 'create' }) => {
  ...
  const loadMaterial = () => {
    ...
    const material = findRecord('materials', 'materialNumber', materialNumber.trim());
    if (material) {
  ...
  const handleSave = () => {
    ...
    try {
      if (mode === 'create') {
        ...
        markAsSaved();
        updateStatus(`Material ${formData.materialNumber} updated successfully`, 'success');
      }
    } catch (error) {
  ...
  const getModeTitle = () => {
    switch (mode) {
      case 'create': return 'Create Material';
      case 'change': return 'Change Material';
      case 'display': return 'Display Material';
      default: return 'Material Master';
    }
  };
      ...
      <div className="sap-panel">
        <div className="sap-panel-header">
          <span>
            ...
            {getModeTitle()} - MM0{mode === 'create' ? '1' : mode === 'change' ? '2' : '3'}
          </span>
        ...
        <div style={{ marginBottom: '16px' }}>
          <input
            ...
            onChange={(e) => {
              ...
              const materials = getTableData('materials');
              const results = materials.filter(m =>
                m.materialNumber?.toLowerCase().includes(e.target.value.toLowerCase()) ||
